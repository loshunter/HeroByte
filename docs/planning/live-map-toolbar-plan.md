...
- Server-side: NOTHING new — `place-room` flows through the existing `map-studio-command` path. **[S1-UPDATE 2026-07-11]** The `authoredDoorIdsOf` command-heuristic this line references was REMOVED during S1's adversarial review (it leaked secret doors). Door-state preservation now compares AUTHORED states between the pre-/post-edit documents (`preserveDoorRuntimeStates` + `authoredDoorStatesOf` in `scenePublish.ts`), so `place-room` needs NO special case — a room's newly-placed doors have no prior authored state and compile at their authored value automatically. Do not re-add a per-command case.
