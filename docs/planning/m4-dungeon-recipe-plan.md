...
2. Rejection-sample room rects: per attempt draw EXACTLY 4 rolls in order — `w = 3 + floor(rng()*7)`, `h = 3 + floor(rng()*7)`, `x = 1 + floor(rng()*(cols-w-1))`, `y = 1 + floor(rng()*(rows-h-1))` — BEFORE any overlap test; accept iff the rect plus a 1-cell margin overlaps no accepted room; stop at `attempts = target * 12` or when the target is met. (Bounds are ≥ 8×8 per G1's validator, so at least one 3×3 room always fits.)
