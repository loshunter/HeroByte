...
1. `packages/shared/src/index.ts`: add to ClientMessage:
   ...
   **[G7-SHIPPED] `secretDoorChance` was REMOVED from this message** — see the banner. Shipped params are `{ theme, density }`, and the zod schema is `.strict()` so a stale client sending the field is rejected loudly rather than silently ignored.
...
### 7.1 Secret doors in generated dungeons — BLOCKED on fog-aware terrain [G7]
...
**Do not** attempt to fix this inside the generator (irregular room shapes, decoy doorless seams, jittered layouts). Those raise the attacker's cost without changing the fact that the floor plan is public; a partial fix here is worse than the honest "not yet", because it re-earns the DM's trust without deserving it.

**Kept, and still tested:** the disguise for HAND-PLACED secret doors (`compiledSceneView.ts` + its hand-authored attack regressions). A hand-drawn map exposes no seam grammar, so the inference has no foothold there.
