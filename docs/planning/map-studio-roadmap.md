...
The roadmap's "AI generation before the deterministic editor is reliable" non-goal below still stands, and this is not that: the dungeon recipe is a **pure, seeded, server-side function**, not a model. A DM arms 🏰 Gen, drags a region, sets theme / density / seed, and a stocked dungeon — rooms, corridors, blocking walls, working doors, brazier lights, and GM-only spawn keys — compiles onto every player's table as **ONE undoable command**. Same seed, same dials → the same dungeon forever, which is what makes Cartridge Codes possible later.

It rides the rails the live-map arc laid: generators emit MapDocument elements (never rasters), so a generated dungeon is editable with every existing tool the moment it lands, and fog/vision/movement-blocking work by construction. See [m4-dungeon-recipe-plan.md](./m4-dungeon-recipe-plan.md) for the slice record, the two spec corrections found in flight, and the gate findings — including four independent attacks on the secret-door disguise.

**Generated dungeons contain no secret doors, on purpose.** Three of those attacks were beaten (G4.5); the fourth could not be. `mapTerrain` ships the floor plan to every player unfiltered, and given the floor plan a doorless room/corridor seam *is* a secret door — recoverable at 202/202 with no false positives regardless of how well the geometry hides it. So the dial was removed rather than shipped as a lie, and the panel says so. **Hand-placed secret doors still work and are still disguised**; restoring the generated ones is blocked on fog-aware terrain (VISION's memory fog) — see §7.1 of the plan.
