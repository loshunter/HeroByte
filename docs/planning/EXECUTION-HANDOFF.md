...
- `db329419` — **the connection-war fix.** Two browser contexts sharing one session uid used to 4001-war forever and neither could authenticate. Now replacement uses close code `WS_CLOSE_REPLACED = 4002` (`packages/shared/src/wsCloseCodes.ts`) — terminal client-side on a direct connection only: Render's proxy rewrites every server-sent close code to 1005 (found 2026-09-20), so in production it was inert until the `connection-closing` data frame (`apps/server/src/ws/announceClosing.ts`, session-identity-binding-arc-plan.md §14.1) made the client go terminal on the frame instead. This was *the* reason custom rooms never worked locally; live, the same war simply continued until that frame landed.
