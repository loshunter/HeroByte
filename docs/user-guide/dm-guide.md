...
**⧉ DUPLICATE** on any NPC card copies that monster — HP, portrait, token art, size, stance and the hidden flag — under the next free number. Temp HP, the initiative modifier and status effects do **not** ride: the copy starts its own fight. Build one goblin the way you want it, then press Duplicate four times. Duplicating **Goblin 3** gives you **Goblin 4** (or 9, if you're already up to 8): it continues the series rather than starting a new one.
