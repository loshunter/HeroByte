...
Adding a token takes a moment because HeroByte renders it down to an 84px thumbnail first and keeps that with the table — which is what stops a 4000px phone photo from being decoded once per cell every time you open the picker. If that cannot be done (some hosts refuse to let another site read their images), the token still works and a line under the form says what was skipped.
