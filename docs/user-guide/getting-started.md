...
That's it — your token appears on the map, and your player card shows up in the **Entities** panel at the bottom. If the table already has a map, the view starts centred on your token, so you are not staring at unexplored fog wondering whether anything loaded. The same happens when you reload.
