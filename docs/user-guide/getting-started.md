...
- **It remembers you.** The password is stored for that table (per browser tab session), so a reload drops you straight back in. If your connection blips mid-game, HeroByte reconnects and re-authenticates automatically — you'll just see a brief `Reconnecting…` banner in the corner. Your browser also keeps a session key for the table, so a reload or a blip brings you back as the same person — DM powers included — for up to six hours. (On the Main Hall the key lasts only until the table wipes itself: see below.)
