...
| **⚒ Tool**         | Opens the sheet: **▶ Start live map** before you have one, then the seven tools below, **👆 Select**, that tool's own dials, and **◇ Recenter**. Picking a tool with no dials closes the sheet, because you picked it in order to use it; a tool that _has_ dials leaves it open so you can set them. |
...
**Seven tools are here** — **🏠 Room**, **🚇 Hall**, **▬ Wall**, **🚪 Door**, **📏 Row**, **〰️ Spline** and **🏰 Gen** — and every one of them is a drag: press, move, lift. That is not a coincidence: the drag tools are the ones a finger can arm safely. **👆 Select** sits beside them and is the one exception — it is a tap, not a drag, and it is there so you can delete something (see below).
