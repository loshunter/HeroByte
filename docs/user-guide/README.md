...
- The **Main Hall** is the default table every server starts with. Until it's given a password of its own it's a **public scratch space** — its password is the documented default, and it wipes itself once it has sat empty for an hour. Giving it a password [claims it](getting-started.md#the-main-hall-is-a-shared-table-until-you-claim-it); on a shared server, use a [private table](getting-started.md#creating-a-private-table) instead.
