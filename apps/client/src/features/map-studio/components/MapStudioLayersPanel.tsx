...
interface MapStudioLayersPanelProps {
  layers: MapLayer[];
  saving: boolean;
...
/**
 * The Layers panel: visibility, lock, opacity, and reordering over the
 * document's layer stack. Rendered top-most layer first, matching how the
 * canvas composites; "up" moves a layer toward the top of the stack (a
 * higher array index — array order IS render order).
 */
export function MapStudioLayersPanel({
  layers,
  saving,
  onUpdateLayer,
  onMoveLayer,
}: MapStudioLayersPanelProps) {
  const stack = layers.map((layer, index) => ({ layer, index })).reverse();
  return (
    <section aria-label="Layers" style={panelStyle}>
      <h3 className="jrpg-text-small" style={{ margin: "0 0 6px" }}>
        LAYERS
      </h3>
      <ul style={listStyle}>
