...
/**
 * ONE context per canvas element, exactly as a browser does. An earlier version
 * handed out a fresh stub per `getContext` call, which made canvas persistence
 * unobservable: the union test grew a module-level log whether or not the paint
 * survived, so "accumulates without clearing" could not fail.
 */
...
function fakeContext() {
  ...
  const stack: number[][] = [];
  let path: ScenePoint[] = [];
  /** What THIS canvas is holding — the thing accumulation is about. */
  const painted: ScenePoint[][] = [];
  return {
    painted,
    fillStyle: "",
    clearRect: vi.fn(() => {
    ...
    closePath: vi.fn(),
    fill: vi.fn(() => {
      fills.push({ transform: [...transform], points: [...path] });
      painted.push([...path]);
    }),
    ...
    getImageData: vi.fn((_x: number, _y: number, w: number, h: number) => {
      // Reflects what THIS canvas still holds, so a mask that was cleared or
      // rebuilt reads back empty and the persistence tests notice.
...
describe("useExploredFog", () => {
  ...
  // THE headline behaviour of S7 part 2: the union is the canvas, nothing is
  // ever cleared between updates, so walking back through a room keeps what was
  // seen before. Asserted on what the canvas still HOLDS (and on its identity),
  // not on a log of calls — a log grows whether or not the paint survives.
  it("accumulates: the second region joins the first on the SAME canvas", () => {
    const { result, rerender } = render([square]);
    const canvas = result.current.canvas!;
    expect(contexts.get(canvas)!.painted).toHaveLength(1);

    rerender({ polygons: [[...square].map((p) => ({ x: p.x + 300, y: p.y }))], storageKey: KEY });

    // Same canvas element — a rebuilt one would silently discard the memory.
    expect(result.current.canvas).toBe(canvas);
    const painted = contexts.get(canvas)!.painted;
    expect(painted).toHaveLength(2);
    expect(painted[0]![1]!.x).toBe(80); // the first room is still there
    expect(painted[1]![1]!.x).toBe(380);
  });

  it("never clears the mask", () => {
    const { rerender } = render([square]);
    rerender({ polygons: [[...square].map((p) => ({ x: p.x + 300, y: p.y }))], storageKey: KEY });
    rerender({ polygons: [[...square].map((p) => ({ x: p.x + 600, y: p.y }))], storageKey: KEY });
  ...
  it("bumps a revision so react-konva repaints pixels that changed under it", () => {
    ...
    const firstCanvas = result.current.canvas;
    ...
    expect(result.current.canvas).toBe(firstCanvas);
  });
