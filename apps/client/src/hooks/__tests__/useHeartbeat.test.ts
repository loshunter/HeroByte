...
function createMockScheduler() {
  const timers = new Set<MockTimer>();

  const createInterval = (callback: () => void, interval: number) => {
    const normalized = interval > 0 ? interval : 0;
    const timer: MockTimer = { callback, interval: normalized, remaining: normalized };
    timers.add(timer);
    return () => {
      timers.delete(timer);
    };
  ...
  const advanceBy = (ms: number) => {
    timers.forEach((timer) => {
      ...
      let remaining = timer.remaining - ms;
      while (remaining <= 0) {
        timer.callback();
  ...
  return { createInterval, advanceBy };
}
    ...
    it("should send initial heartbeat immediately on mount when enabled", () => {
      ...
      renderHook(() =>
        useHeartbeat({
          sendMessage,
          enabled: true,
          createInterval: scheduler.createInterval,
        }),
      );
