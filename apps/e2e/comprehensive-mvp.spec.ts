...
async function elevateToDM(page: Page) {
  ...
  if (await settingsIcon.isVisible({ timeout: 2000 })) {
    ...
    if (await dmOption.isVisible({ timeout: 2000 })) {
      ...
      const dmPasswordInput = page.locator('input[placeholder*="DM" i], input[type="password"]').first();
...
test.describe("HeroByte Comprehensive MVP Tests", () => {
  ...
  test("2. Drawing Tools - Freehand drawing works", async ({ page }) => {
    ...
    const drawButton = page.locator('button:has-text("Draw")').or(
      page.locator('[title*="Draw" i]')
    ).first();
  ...
  test("3. Partial Erase - Can erase part of a drawing", async ({ page }) => {
    ...
    if (await drawButton.isVisible()) {
      ...
      if (box) {
        ...
        // Switch to eraser
        const eraseButton = page.locator('button:has-text("Erase")').or(
          page.locator('[title*="Erase" i]')
        ).first();
  ...
  test("4. Multi-Select - Can select and manipulate multiple objects", async ({ page }) => {
    ...
    const selectButton = page.locator('button:has-text("Select")').or(
      page.locator('[title*="Select" i]')
    ).first();
  ...
  test("5. Dice Rolling - Can roll dice and see results", async ({ page }) => {
    ...
    if (await diceButton.isVisible()) {
      ...
      const d20Button = page.locator('button:has-text("d20")').or(
        page.locator('button:has-text("D20")')
      ).first();
  ...
  test("6. Session Save/Load - Can save and reload session", async ({ page, context }) => {
    ...
    const dmMenuButton = page.locator('button:has-text("DM Menu")').or(
      page.locator('[title*="DM Menu" i]')
    ).first();
  ...
  test("7. Two-Browser Sync - Two clients see each other's actions", async ({ browser }) => {
    ...
    try {
      ...
      const drawButton = page1.locator('button:has-text("Draw")').or(
        page1.locator('[title*="Draw" i]')
      ).first();
  ...
  test("8. Voice Chat - Voice indicator appears", async ({ page }) => {
    ...
    const hasMicButton = await page.locator('button:has-text("Mic")').or(
      page.locator('[title*="Microphone" i]').or(
        page.locator('svg[data-icon*="microphone" i]')
      )
    ).count();
