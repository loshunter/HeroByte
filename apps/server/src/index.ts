...
import type { Token, RoomSnapshot, Player, Pointer, Drawing } from "@shared";
...
function saveState() {
  try {
    writeFileSync(
      STATE_FILE,
      JSON.stringify({ tokens, players, mapBackground, drawings, gridSize }, null, 2)
    );
  } catch (err) {
...
function broadcast() {
  ...
  const snap: RoomSnapshot = { users, tokens, players, mapBackground, pointers, drawings, gridSize };
...
// Create WebSocket server on port 8787, accessible from any network interface
