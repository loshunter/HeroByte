...
**Implementation**:
- [x] Update pointer data structure to include: `{ id, x, y, name, timestamp, uid }`
- [x] Change from single pointer to array of temporary pointers in state
- [x] Broadcast pointer creation with timestamp
- [x] Client-side: Use `setTimeout` to remove pointer after 3s based on timestamp
- [x] Create CSS keyframe animation:
  ```css
  ...
  ```
- [x] Update PointersLayer component to handle multiple pointers with animation
- [x] Update usePointerTool hook to toggle off when clicked again
- [x] Remove old pointer cleanup logic (no more single persistent pointer)
...
- [ ] Click pointer multiple times rapidly → verify all appear and expire independently
  - [x] Verify 3-second lifespan timing
  - [x] Verify pulse animation smoothness
  - [x] Test with multiple clients → all see same pointers expire simultaneously
- [x] Click pointer button while in pointer mode → should cancel/exit mode
