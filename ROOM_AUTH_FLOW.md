...
1. **Connect.** A newcomer claiming a uid whose session is live and authenticated on another socket is _held_: not registered, and the incumbent is not closed. A dead or unauthenticated occupant is replaced (the reconnect-after-blip path), and adoption never confers auth — the uid's flag and session are cleared, the token record detached.
...
3. **Authenticate.** After the room password verifies, a live authenticated occupant keeps the slot unless the newcomer's `token` matches the session's record. Then it is the same session (a reconnect, or a second tab of the same browser): the old socket is closed with `WS_CLOSE_REPLACED` (4002) and the newcomer takes over. Otherwise the newcomer is closed with `WS_CLOSE_SESSION_CONFLICT` (4003) and the occupant is untouched. Separately, the persisted `isDM` flag is restored only when the token matches _for that table_; a tokenless reclaim gets the uid's record as a non-DM.
